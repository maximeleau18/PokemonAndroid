/**************************************************************************
 * PersonnageNonJoueurListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display PersonnageNonJoueur entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a PersonnageNonJoueur list. A click on an item will get 
 * you to PersonnageNonJoueurShowActivity.
 * - On a tablet, the PersonnageNonJoueurShowFragment will be displayed right next to
 * the list.
 */
public class PersonnageNonJoueurListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected PersonnageNonJoueurListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected PersonnageNonJoueurShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (PersonnageNonJoueurShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (PersonnageNonJoueurListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_personnagenonjoueur_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, PersonnageNonJoueurShowActivity.class);
            final PersonnageNonJoueur item = PersonnageNonJoueurContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(PersonnageNonJoueurContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(PersonnageNonJoueur item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            PersonnageNonJoueur item = PersonnageNonJoueurContract.cursorToItem(
