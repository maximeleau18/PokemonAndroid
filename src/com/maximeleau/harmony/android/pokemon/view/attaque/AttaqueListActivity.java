/**************************************************************************
 * AttaqueListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Attaque entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Attaque list. A click on an item will get 
 * you to AttaqueShowActivity.
 * - On a tablet, the AttaqueShowFragment will be displayed right next to
 * the list.
 */
public class AttaqueListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected AttaqueListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected AttaqueShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (AttaqueShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (AttaqueListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_attaque_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, AttaqueShowActivity.class);
            final Attaque item = AttaqueContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(AttaqueContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Attaque item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Attaque item = AttaqueContract.cursorToItem(
