/**************************************************************************
 * TypeDePokemonEvolutionListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display TypeDePokemonEvolution entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a TypeDePokemonEvolution list. A click on an item will get 
 * you to TypeDePokemonEvolutionShowActivity.
 * - On a tablet, the TypeDePokemonEvolutionShowFragment will be displayed right next to
 * the list.
 */
public class TypeDePokemonEvolutionListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected TypeDePokemonEvolutionListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected TypeDePokemonEvolutionShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (TypeDePokemonEvolutionShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (TypeDePokemonEvolutionListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_typedepokemonevolution_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, TypeDePokemonEvolutionShowActivity.class);
            final TypeDePokemonEvolution item = TypeDePokemonEvolutionContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(TypeDePokemonEvolutionContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(TypeDePokemonEvolution item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            TypeDePokemonEvolution item = TypeDePokemonEvolutionContract.cursorToItem(
