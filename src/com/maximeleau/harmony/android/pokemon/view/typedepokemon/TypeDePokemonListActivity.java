/**************************************************************************
 * TypeDePokemonListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display TypeDePokemon entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a TypeDePokemon list. A click on an item will get 
 * you to TypeDePokemonShowActivity.
 * - On a tablet, the TypeDePokemonShowFragment will be displayed right next to
 * the list.
 */
public class TypeDePokemonListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected TypeDePokemonListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected TypeDePokemonShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (TypeDePokemonShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (TypeDePokemonListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_typedepokemon_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, TypeDePokemonShowActivity.class);
            final TypeDePokemon item = TypeDePokemonContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(TypeDePokemonContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(TypeDePokemon item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            TypeDePokemon item = TypeDePokemonContract.cursorToItem(
