/**************************************************************************
 * TypeObjetListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display TypeObjet entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a TypeObjet list. A click on an item will get 
 * you to TypeObjetShowActivity.
 * - On a tablet, the TypeObjetShowFragment will be displayed right next to
 * the list.
 */
public class TypeObjetListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected TypeObjetListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected TypeObjetShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (TypeObjetShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (TypeObjetListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    protected int getContentView() {
        return R.layout.activity_typeobjet_list;
    }
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, TypeObjetShowActivity.class);
            final TypeObjet item = TypeObjetContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(TypeObjetContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(TypeObjet item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            TypeObjet item = TypeObjetContract.cursorToItem(
