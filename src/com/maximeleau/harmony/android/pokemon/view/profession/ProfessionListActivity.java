/**************************************************************************
 * ProfessionListActivity.java, pokemon Android
 *
 * Copyright 2016
 * Description : 
 * Author(s)   : Harmony
...
/**
 * This class will display Profession entities in a list.
 *
 * This activity has a different behaviour wether you're on a tablet or a phone.
 * - On a phone, you will have a Profession list. A click on an item will get 
 * you to ProfessionShowActivity.
 * - On a tablet, the ProfessionShowFragment will be displayed right next to
 * the list.
 */
public class ProfessionListActivity 
        extends HarmonyFragmentActivity
        implements HarmonyListFragment.OnClickCallback,
                HarmonyListFragment.OnLoadCallback {

    /** Associated list fragment. */
    protected ProfessionListFragment listFragment;
    /** Associated detail fragment if any (in case of tablet). */
    protected ProfessionShowFragment detailFragment;
    ...
    protected void onPostCreate(Bundle savedInstanceState) {
        ...
        this.detailFragment = (ProfessionShowFragment) 
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_show);
        this.listFragment = (ProfessionListFragment)
                        this.getSupportFragmentManager().findFragmentById(
                                R.id.fragment_list);
    ...
    public void onListItemClick(ListView l, View v, int position, long id) {
        ...
        if (this.isDualMode()) {
            ...
            final Intent intent = new Intent(this, ProfessionShowActivity.class);
            final Profession item = ProfessionContract
                    .cursorToItem((android.database.Cursor)
                            l.getItemAtPosition(position));
            Bundle extras = new Bundle();
            extras.putParcelable(ProfessionContract.PARCEL, item);
     ...
     */
    private void loadDetailFragment(Profession item) {
    ...
    private void selectListItem(int listPosition) {
        ...
        if (listSize > 0) {
            ...
            Profession item = ProfessionContract.cursorToItem(
