...
public abstract class TypeDePokemonEvolutionProviderAdapterBase
    ...
    public int delete(
        ...
        switch (matchedUri) {
            case TYPEDEPOKEMONEVOLUTION_ONE:
                String evolueEnId = uri.getPathSegments().get(1);
                String estEvolueEnId = uri.getPathSegments().get(2);
                selection = TypeDePokemonEvolutionContract.COL_EVOLUEEN_ID
                        + " = ?"
                        + " AND "
                        + TypeDePokemonEvolutionContract.COL_ESTEVOLUEEN_ID
                        + " = ?";
                selectionArgs = new String[2];
                selectionArgs[0] = evolueEnId;
                selectionArgs[1] = estEvolueEnId;
        ...
        switch (matchedUri) {
            case TYPEDEPOKEMONEVOLUTION_ALL:
                if (values.size() > 0) {
                    ...
                    id = (int) this.adapter.insert(TypeDePokemonEvolutionContract.COL_EVOLUEEN_ID, values);
                }
                if (id > 0) {
                    result = Uri.withAppendedPath(
                            TYPEDEPOKEMONEVOLUTION_URI,
                            values.getAsString(TypeDePokemonEvolutionContract.COL_EVOLUEEN_ID)
                            + "/"
                            + values.getAsString(TypeDePokemonEvolutionContract.COL_ESTEVOLUEEN_ID));
                }
                break;
            default:
                result = null;
    ...
    public int update(
        ...
        switch (matchedUri) {
            case TYPEDEPOKEMONEVOLUTION_ONE:
                ...
                selection += " AND " + TypeDePokemonEvolutionContract.COL_ESTEVOLUEEN_ID + " = ?";
     ...
     * @return The cursor
     */
    private android.database.Cursor queryById(String evolueEnId, String estEvolueEnId) {
        android.database.Cursor result = null;
