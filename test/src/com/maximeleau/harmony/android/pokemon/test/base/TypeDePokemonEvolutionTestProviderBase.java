...
public abstract class TypeDePokemonEvolutionTestProviderBase extends TestDBBase {
    ...
    public void testCreate() {
        ...
        if (this.entity != null) {
            ...
            Assert.assertTrue(Integer.parseInt(result.getPathSegments().get(1)) == typeDePokemonEvolution.getEvolueEn().getId());
            Assert.assertTrue(Integer.parseInt(result.getPathSegments().get(2)) == typeDePokemonEvolution.getEstEvolueEn().getId());
            ...
            try {
                android.database.Cursor c = this.provider.query(Uri.parse(
                        TypeDePokemonEvolutionProviderAdapter.TYPEDEPOKEMONEVOLUTION_URI
                                + "/" 
                                + this.entity.getEvolueEn().getId()
                                + "/" 
                                + this.entity.getEstEvolueEn().getId()),
            ...
            try {
                ...
                result = this.provider.update(
                    Uri.parse(TypeDePokemonEvolutionProviderAdapter.TYPEDEPOKEMONEVOLUTION_URI
                        + "/"
                        + typeDePokemonEvolution.getEvolueEn().getId()
                        + "/"
                        + typeDePokemonEvolution.getEstEvolueEn().getId()),
                    values,
                    null,
                    null);
    ...
    public void testUpdateAll() {
        ...
        if (this.entities != null) {
            if (this.entities.size() > 0) {
                ...
                try {
                    ContentValues values = TypeDePokemonEvolutionContract.itemToContentValues(typeDePokemonEvolution);
                    values.remove(TypeDePokemonEvolutionContract.COL_EVOLUEEN_ID);
                    values.remove(TypeDePokemonEvolutionContract.COL_ESTEVOLUEEN_ID);
